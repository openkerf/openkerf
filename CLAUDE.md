...
| **Connecting a Ruida runs an empty function** | `ruida/device.py:452` → `ruida/ruidasession.py:91` | `ruida_connect` is the only connect command the device registers, and all it does is `active_session.open()` — whose whole body is `'''This is a stub because the connect opens the transport.''' return`. The real handshake is `RuidaSession.connect()`, and its own docstring says it is "intended to run only in the context of the handshaker or status monitor threads". So the button cannot open anything: measured against the stack beside the laser, `POST /api/machine/connect` returned 409 with an empty console channel — no error, because nothing failed and nothing happened. What does rebuild the session is `.interface_update`, and only when `service.interface` differs from the session's (`:401`); the session snapshots that value at construction (`ruidasession.py:44`), so setting the interface anywhere but the wxPython config panel never reaches it — there is no `signal_listener` for it. Measured: with the setting on `udp` the live session still held a `USBTransport`, and `.interface_update` swapped it for a `UDPTransport` that bound port 40200. |
| **A UDP transport that fails to bind is never retried** | `ruida/udp_transport.py:37` → `ruida/ruidasession.py:98`, `:263` | `UDPTransport.open()` binds port 40200 without `SO_REUSEADDR` and raises `TransportError` on `OSError`; `_open()` swallows it and returns, and the handshaker's retry — the branch that closes and reopens the transport — sits behind `if self.interface == 'usb'`. On UDP it therefore never runs. One failed bind is permanent: the session exists, `connected` stays false for the life of the process, nothing is bound anywhere in the namespace, and not one line reaches the log. Measured on the Docker stack beside the laser: after a container **restart** the machine read `disconnected` for good with nothing on UDP 40200 in the host namespace; a full `down` + `up` had the port bound and the state connected within seconds. A restart that overlaps the old process holding the port is enough to cause it, and the Connect button above cannot repair it. |
| **A Ruida stops answering for seconds at a time, and comes back** | the machine, not the code | Measured on a real KH-5030 over UDP, three minutes of an idle connection sampled every 0.3 s: four gaps in which `connected` read false, holding 3.9, 4.5 and 4.6 s and once less than a sample. `tcpdump` on the host through one of them: seven of our ENQ packets out of `40200 > 50200`, not one answer, then replies again with nothing in between to explain it — and the ARP re-resolution in the middle starts three seconds *after* the silence, so it is the consequence and not the cause (a permanent `ip neigh` entry for the machine changed nothing). The engine turns one missed reply into `_responding = False` (`ruidasession.py:409`) and the status monitor then reconnects, so the flag flaps. Our layer waits such a gap out rather than believing it: `RuidaUpload.line_gap_seconds` and `LINE_GAP_MS`, both 8 s, above every gap measured. |
| **`RuidaController.__init__` starts a thread that redials forever** | `ruida/controller.py:50`, `:189` | Not "once after 3 s": the thread sleeps 3 s and then, every 0.2 s forever (five times a second), calls `service.connect()` whenever the service isn't both connected and idle. Measured against a driver built purely to read bytes off, never connected: 6 calls in a 4.2 s wait that starts right after construction — the first ~3 s of that wait is the startup sleep, so this is the 5/s rate over the ~1.2 s of it that is actually active. A driver kept around for more than one read leaves this running for the life of the process. We report such a driver's service as always connected and never busy (a thin wrapper, `openkerf_api/commands.py`), which measured 0 calls in the same window — but its own `_job_lock`, acquired at `:49` and released only by `resume_monitor` (`:95`), is shared with every `_data_sender` a later job starts (`ruida/driver.py:301`); left unreleased, ten reads left ten such threads permanently blocked on it (still alive 3 s later) even though the reconnect storm was gone. Releasing that lock once fixed both: measured, ten reads then leave one extra thread total, the same as one. |
