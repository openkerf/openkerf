...
def test_a_sheet_can_be_renamed_and_resized(client):
    client.patch("/api/sheets/sheet-1", json={"name": "Berken 3mm", "width_mm": 300})
...
def test_a_job_carries_the_sheet_name_after_switching_sheets(kernel, client):
    ...
    wisselen van vel laadt `sheet-1.svg` terug, en die interne bestandsnaam ging
    als jobnaam de wachtrij in. Bij de machine staan dan jobs die `sheet-1.svg`
...
def test_a_restart_comes_back_to_the_sheet_you_left(kernel, tmp_path):
    ...
    alles — wegwisselen ziet een lege boom en gooit `sheet-1.svg` weg.
    """
